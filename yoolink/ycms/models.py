...
def unique_image_name(instance, filename):
    ...
    return f"wunkschweiss/images/{slugify(base)}_{timestamp}{ext}"
...
def upload_to_galery_image(instance, filename):
    ...
    timestamp = timezone.now().strftime("%Y%m%d%H%M%S")
    base, ext = os.path.splitext(filename)
    return f"wunkschweiss/galeryImages/{slugify(base)}_{timestamp}{ext}"
...
def upload_to_blog_image(instance, filename):
    return f"wunkschweiss/blogs/{instance.id}/{filename}"
def default_code():
    return dict()
...
def upload_to_product_image(instance, filename):
    return f"wunkschweiss/products/{instance.id}/{filename}"
...
class Product(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("cms:product-detail", kwargs={"pk": self.pk, "slug_title": self.slug})
